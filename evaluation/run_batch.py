...
Records estimated offset, confidence score, and runtime for each.
...
def _run_visual_sync(original_path: str, synthetic_path: str) -> dict:
    """
    ...
    try:
        ...
        # For confidence, re-run pairwise correlation to extract the score
        from src.visual_sync import (
        ...
        try:
            m1, fps1 = extract_motion_energy(
                os.path.join(video_dir, orig_fname), step=3
            )
            m2, fps2 = extract_motion_energy(
                os.path.join(video_dir, synth_fname), step=3
            )
            m1 = smooth_motion_signal(m1, fps1)
            m2 = smooth_motion_signal(m2, fps2)

            target_fps = 10.0
            m1 = resample(m1, int(len(m1) * target_fps / fps1)) if fps1 > 0 else m1
            m2 = resample(m2, int(len(m2) * target_fps / fps2)) if fps2 > 0 else m2
...
def run_batch(
    ...
    for idx, entry in enumerate(metadata, 1):
        ...
        try:
            ...
            logger.info(
                "    Audio: est=%+.1fms  err=%.1fms  conf=%.3f  time=%.2fs",
        ...
        try:
            ...
            visual_result = _run_visual_sync(original_path, synthetic_path)
            ...
            logger.info(
                "    Visual: est=%+.1fms  err=%.1fms  conf=%.3f  time=%.2fs",
